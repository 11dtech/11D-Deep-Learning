...
-- build simple recurrent neural networkSi
...
progress_earn = torch.Tensor(maxIteration):fill(0)
 
--training
local index = 1
local iteration = 1
while iteration <= maxIteration do
	local train_err,test_err,true_err,earn = 0,0,0,0
	for index = 1,((#signal)[1]-rho+1-lead) do --skip the prediciton after the last point
   	...
   	rnn:forget() --forget all past time-steps
		...
		local valid = (outputs[rho][1][1]-signal[index+rho-1][2])/(trend[index+rho-1+lead][2]-trend[index+rho-1][2])  --validate the correctness of prediction
		earn = earn + torch.sign(torch.DoubleTensor({valid}))[1]
	...
	local Earning =  earn/((#signal)[1]-rho)
	progress_earn[{iteration}] = Earning

	print(string.format("Iteration %d; Mean MSE Training Err = %f; Mean MSE True Err = %f; Err Diff = %f; Earning = %f", iteration, AE_train, AE_true, diff, Earning))
	
	if iteration > 1 then
	gnuplot.figure(1)	
		gnuplot.plot({'Signal', signal[{ {1,500}, {} }], '-'}, {'Trend', trend[{ {1,500}, {} }], '~'}, {'RNN', prediction[{ {1,500}, {} }], 'with points ps 0.75'})
	...
	gnuplot.figure(3)
		gnuplot.plot({'Earning', progress_earn})
			gnuplot.xlabel('Timestep')
			gnuplot.ylabel('Earning')
			gnuplot.title('Earning Progress')
			gnuplot.grid(true)	
	end
